...
# ModelOutput是一个用于统一模型输出格式的基类，可以帮助用户更容易地处理模型返回的数据。
from transformers.file_utils import ModelOutput
# snapshot_download函数用于下载Hugging Face Hub上某个仓库的快照
...
class EncoderOutput(ModelOutput):
    q_reps: Optional[Tensor] = None # 查询的表示向量
    p_reps: Optional[Tensor] = None # 文档的表示向量
    loss: Optional[Tensor] = None # 存储模型的损失值
    scores: Optional[Tensor] = None # 评分或相似度度量
...
class BGEM3Model(nn.Module):
    ...
    def __init__(self,
                 ...
                 negatives_cross_device: bool = False, # 是否跨设备使用负样本
                 temperature: float = 1.0,
                 enable_sub_batch: bool = True, # 是否启用子批处理
                 unified_finetuning: bool = True, # 是否启用统一微调
                 use_self_distill: bool = False, # 是否使用自我蒸馏
                 colbert_dim: int = -1, # ColBERT向量维度
                 self_distill_start_step: int = -1, # 自我蒸馏开始步骤
                 ):
        super().__init__()
        # 加载指定名称的模型，并且可以指定ColBERT向量的维度。
        ...
        self.step = 0
        if not normlized:
            # 如果不进行标准化，则将温度参数重置为1.0，并记录一条信息日志。
        ...
        if self.negatives_cross_device:
            # 如果启用了跨设备负样本处理，接下来的代码会检查分布式训练是否已初始化。
            # 如果没有初始化，则抛出一个值错误异常。
            if not dist.is_initialized():
                raise ValueError('Distributed training has not been initialized for representation all gather.')
            # 如果分布式训练已初始化，这两行代码会获取当前进程的排名和总进程数
